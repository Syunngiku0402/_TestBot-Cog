...
class ctest(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot

    @app_commands.command(name="ccitemnum-change", description="実行するとアイテム数を変換してくれます")
    ...
    @app_commands.command(name="ctest", description="テスト用コマンド")
    @app_commands.describe(file="ファイルを添付してください")
    async def test(self, interaction: discord.Interaction, file: discord.Attachment):
        await interaction.response.defer(thinking=True)
        # fileをローカルに保存
        await interaction.followup.send(f"ファイル名: {file.filename}\nファイルサイズ: {file.size} bytes\nファイルURL: {file.url}")


async def setup(bot: commands.Bot):
    await bot.add_cog(ctest(bot))
